...
import { htmlFormat } from '../../transformations/formatting'
...
interface HtmlFormatPageProps {
  tabId: string
}

const HtmlFormatPage: React.FC<HtmlFormatPageProps> = ({ tabId }): React.ReactElement => {
  ...
  const applyTransformation = async (): Promise<void> => {
    ...
    try {
      // Apply the transformation
      const result = await htmlFormat(inputText, { indentSize })
  ...
  return (
    <div className="flex flex-col h-full p-4 bg-background text-text overflow-y-auto">
      <div className="mb-6 pb-4 border-b border-border">
        <h1 className="mb-2 text-[1.8rem]">HTML Formatter</h1>
        <p className="text-text text-base">Format HTML code with proper indentation</p>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 flex-1 min-h-0">
        ...
        <div className="relative flex-1 flex flex-col">
          <TextAreaWithLabel
            ...
            spellCheck="false"
          />
          {isTransforming && <TransformationAnimation transformationName="HTML Format" />}
        </div>
      </div>
    </div>
  )
