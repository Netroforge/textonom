          ...
          >
            {{
              recordingAction === actionId
                ? 'Press keys...'
                : hotkeysStore.getHotkeyString(actionId) || 'Not set'
            }}
          </div>
          <button class="hotkey-reset" title="Reset to default" @click="resetHotkey(actionId)">
            ↺
          </button>
        </div>
      </div>
...
const handleClickOutside = (event) => {
  if (recordingAction.value && !event.target.closest('.hotkey-display')) {
    recordingAction.value = null
  }
}
